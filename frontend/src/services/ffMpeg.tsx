...
const handleDocumentSelection = async (userId: any) => {
    try {
...
const postVideo = async (file: any, userId: any) => {
...
const FFmpegWrapper = {
    handleVideoLoad: async function handliVideoLoad(context: any) {
        const codecs = ['mp4', 'mov'];
        if (codecs.some(codec => context.video.uri.toLowerCase().includes(codec))) {
            console.log('No need to convert');
        } else {
            console.log('Converting');
            const input = await FFmpegKitConfig.getSafParameterForRead(
                extension + context.video.uri,
            );
            const output = await FFmpegKitConfig.selectDocumentForWrite(
                context.video.name + '.mp4',
                'video/*',
            ).then(res => {
            ...
            FFmpegKit.execute(`-i ${input} -c:v mpeg4 ${output}`).then(
                async session => {
    ...
    changeAudio: async function changeAudio(context: any, delay: string, userId: any) {
        const input = await FFmpegKitConfig.getSafParameterForRead(
            extension + context.video.uri,
        );
        const output = await FFmpegKitConfig.selectDocumentForWrite(
            context.video.name.split('.')[0] + '_audio_overlaid' + '.mp4',
            'video/*',
        ).then(res => {
            return FFmpegKitConfig.getSafParameterForWrite(res);
        });
        const audio = await FFmpegKitConfig.selectDocumentForRead().then(res => {
            return FFmpegKitConfig.getSafParameterForRead(res);
        });
        console.log('input: ', input);
        console.log('output: ', output);
        ...
        FFmpegKit.execute(cmd).then(async session => {
            ...
            if (ReturnCode.isSuccess(returnCode)) {
                ...
                context.dispatch({
                    type: 'SET_VIDEO',
                    payload: {
                        name: context.video.name.split('.')[0] + '_audio_overlaid',
                        uri: output,
                    },
                });
            } else if (ReturnCode.isCancel(returnCode)) {
    ...
    mirrorVideo: async function mirrorVideo(context: any, userId: any) {
        const input = await FFmpegKitConfig.getSafParameterForRead(
            extension + context.video.uri,
        );
        const output = await FFmpegKitConfig.selectDocumentForWrite(
            context.video.name.split('.')[0] + '_mirrored' + '.mp4',
            'video/*',
        ).then(res => {
            return FFmpegKitConfig.getSafParameterForWrite(res);
        });
        console.log('input: ', input);
        console.log('output: ', output);
        FFmpegKit.execute(`-i ${input} -vf "hflip" ${output}`).then(
            async session => {
                const state = FFmpegKitConfig.sessionStateToString(
                    await session.getState(),
                );
                ...
                if (ReturnCode.isSuccess(returnCode)) {
                    ...
                    context.dispatch({
                        type: 'SET_VIDEO',
                        payload: {
                            name: context.video.name.split('.')[0] + '_mirrored',
                            uri: context.video.name.split('.')[0] + '_mirrored' + '.mp4',
                        },
                    });
                } else if (ReturnCode.isCancel(returnCode)) {
                    console.log('Encode canceled');
