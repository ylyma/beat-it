...
const TracksDisplayComponent = () => {
    ...
    useEffect(() => {
        const getPlaylistItems = async (title: any) => {
            await fetch(
        ...
        <ScrollView>
            ...
            <View style={styles.playRow}>
                <TouchableOpacity
                    ...
                    <Ionicons name={'play-skip-back'} size={35} color={colors.yellow} />
                </TouchableOpacity>
                <TouchableOpacity style={styles.play} onPress={() => playPlaylist()}>
                    <Ionicons name={'reload'} size={40} color={colors.secondary} />
                </TouchableOpacity>
                <TouchableOpacity
                    style={styles.play}
                    onPress={() => {
                    ...
                    <Ionicons name={'play-skip-forward'} size={35} color={colors.yellow} />
                </TouchableOpacity>
            </View>
            <View style={styles.wordRow}>
                <Text style={[styles.word, { color: colors.text }]}>restart</Text>
            </View>
            {tracks[0] !== '' ? (
                tracks.map(track => (
                    <View key={track}>
            ...
            <View style={styles.bottom}>
                ...
                {queue[0] !== '' ? (
                    queue.map(track => (
                        <View key={track}>
                            <Text style={[styles.queueItem, { color: colors.alwaysblack }]}>{track}</Text>
                        </View>
                    ))
                ) : (
                    <View />
