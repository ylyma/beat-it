...
type Props = { userId: string; refresh: boolean; reload: () => {}; search: string };
...
const VideosContainer: (props: Props) => ReactElement = ({
    ...
    return (
        <View>
            {videos[0] !== '' ? (
                videos.map(track => search == undefined ?
                    <View key={track}>
                        <VideoButton videoName={track} userId={userId} reload={reload} />
                    </View> : track.toLowerCase().includes(search) ?
                        <View key={track}>
                            <VideoButton videoName={track} userId={userId} reload={reload} />
                        </View> : <View></View>
                )
